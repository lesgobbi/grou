...
class AdminCategory {
    ...
    public function ExeCreate(array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Result = false;
            $this->Error = ['<b>Erro ao cadastrar:</b> Para cadastrar uma categoria, preencha todos os campos!', ALERT];
        else:
            $this->setData();
            $this->setName();
            
            $position = new Read;
            if($this->Data['category_parent']):
                $position->FullRead("SELECT MAX(category_order) FROM ".self::Entity." WHERE category_parent = :parent", "parent={$this->Data['category_parent']}");
            else:
                $position->FullRead("SELECT MAX(category_order) FROM ".self::Entity." WHERE category_parent IS NULL");
            endif;
            $this->Data['category_order'] = $position->getResult()[0]['MAX(category_order)'] + 1;
    ...
    public function ExeUpdate($CategoryId, array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Result = false;
            $this->Error = ["<b>Erro ao atualizar:</b> Para atualizar a categoria {$this->Data['category_title']}, preencha todos os campos!", ALERT];
        else:
            $this->setData();
            $this->setName();
            $this->Update();
    ...
    public function ExeDelete($CategoryId) {
        ...
        if (!$read->getResult()):
            $this->Result = false;
            $this->Error = ['Oppsss, você tentou remover uma categoria que não existe no sistema!', INFOR];
        else:
            extract($read->getResult()[0]);
            if (!$category_parent && !$this->checkCats($this->CatId)):
                $this->Result = false;
                $this->Error = ["A <b>seção {$category_title}</b> possui categorias cadastradas. Para deletar, antes altere ou remova as categorias!", ALERT];
            elseif ($category_parent && !$this->checkPosts()):
                $this->Result = false;
                $this->Error = ["A <b>categoria {$category_title}</b> possui artigos cadastrados. Para deletar, antes altere ou remova todos os posts desta categoria!", ALERT];
            else:
                $delete = new Delete;
                ...
                $this->Result = true;
                $this->Error = ["A <b>{$tipo} {$category_title}</b> foi removida com sucesso do sistema!", ACCEPT];
            endif;
        endif;
    }
    ...
    private function checkPosts() {
        $readPosts = new Read;
        $readPosts->ExeRead("posts", "WHERE post_category = :category", "category={$this->CatId}");
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            ...
            $this->Error = ["<b>Sucesso:</b> A categoria {$this->Data['category_title']} foi cadastrada no sistema!", ACCEPT];
        endif;
    }
    ...
    private function Update() {
        ...
        if ($Update->getResult()):
            ...
            $this->Result = true;
            $this->Error = ["<b>Sucesso:</b> A <strong>{$tipo} {$this->Data['category_title']}</strong> foi atualizada no sistema!", ACCEPT];
        endif;
    }
