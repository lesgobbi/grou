...
class AdminUser {
    ...
    public function ExeDelete($UserId) {
        ...
        if (!$readUser->getResult()):
            $this->Error = ['Oppsss, você tentou remover um usuário que não existe no sistema!', ERROR];
            $this->Result = false;
        elseif ($this->User == $_SESSION['userlogin']['user_id']):
            $this->Error = ['Oppsss, você tentou remover seu usuário. Essa ação não é permitida!!!', INFOR];
            $this->Result = false;
        else:
            if ($readUser->getResult()[0]['user_level'] == 3):
                ...
                if (!$readAdmin->getRowCount()):
                    $this->Error = ['Oppsss, você está tentando remover o único ADMIN do sistema. Para remover cadastre outro antes!!!', ERROR];
                    $this->Result = false;
                else:
                    $this->Delete();
                endif;
    ...
    private function checkData() {
        if (in_array('', $this->Data)):
            $this->Error = ["Existem campos em branco. Favor preencha todos os campos!", ALERT];
            $this->Result = false;
        elseif (!Check::Email($this->Data['user_email'])):
            $this->Error = ["O e-email informado não parece ter um formato válido!", ALERT];
            $this->Result = false;
        else:
            $this->checkName();
            $this->checkEmail();
    ...
    private function checkEmail() {
        ...
        if ($readUser->getRowCount()):
            $this->Error = ["O e-email informado foi cadastrado no sistema por outro usuário! Informe outro e-mail!", ERROR];
            $this->Result = false;
        else:
            $this->Result = true;
        endif;
    ...
    //Verifica usuário pelo nome, Impede cadastro duplicado!
    private function checkName() {
        $Where = ( isset($this->User) ? "user_id != {$this->User} AND" : '');

        $readUser = new Read;
        $readUser->ExeRead(self::Entity, "WHERE {$Where} user_name = :name", "name={$this->Data['user_name']}");

        if ($readUser->getRowCount()):
            $this->Error = ["O nome informado foi cadastrado no sistema por outro usuário! Informe outro nome!", ERROR];
            $this->Result = false;
        else:
            $this->Result = true;
        endif;
    ...
    private function Create() {
        ...
        $this->Data['user_password'] = password_hash($this->Data['user_password'], PASSWORD_DEFAULT, ['cost' => 11]);

        $Create->ExeCreate(self::Entity, $this->Data);

        if ($Create->getResult()):
            $this->Error = ["O usuário <strong>{$this->Data['user_name']}</strong> foi cadastrado com sucesso no sistema!", ACCEPT];
    ...
    private function Update() {
        ...
        $this->Data['user_lastupdate'] = Check::Data(date('d/m/Y H:i:s'));

        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE user_id = :id", "id={$this->User}");
        if ($Update->getResult()):
            $this->Error = ["O usuário <strong>{$this->Data['user_name']}</strong> foi atualizado com sucesso!", ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if ($Delete->getResult()):
            $this->Error = ["Usuário removido com sucesso do sistema!", ACCEPT];
            $this->Result = true;
        endif;
    }
