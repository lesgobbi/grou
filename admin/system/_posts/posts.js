...
(function () {
    ...
    $(function () {
        ...
        $('#img-cover').on('click', function () {
            ...
            upload(990, 250, '#post_cover');
        });

        $('#img-featured').on('click', function () {
            $('#upload-crop').croppie('destroy');
            upload(400, 300, '#post_cover_featured');
        });
...
//                    alert(data);
        ...
        if ($('#post_content').length) {
            ...
            $('#post_content').froalaEditor({
                language: 'pt_br',
                fileUploadURL: 'bower_components/editor/upload_file.php',
                imageUploadURL: 'bower_components/editor/upload_image.php',
                toolbarButtons: ['bold', 'italic', 'underline', 'fontSize', 'color', '|', 'insertLink', 'insertImage', 'insertVideo', 'insertFile', 'insertTable', '-', 'align', 'formatOL', 'formatUL', 'outdent', 'indent', '|', 'insertHR', 'undo', 'redo', 'selectAll', 'clearFormatting'],
                colorsStep: 8,
                colorsText: colors,
        ...
        $('.switch').on('change', function () {
            ...
            var checkType = type.indexOf("featured");
            ...
            if (checkType === 0) {
                if (this.checked) {
                    ...
                    setTimeout(function () {
                        location.href = "painel.php?exe=posts/posts&post=" + id + "&action=nonfeatured";
                    }, 200);
                }
            } else {
                if (this.checked) {
                    setTimeout(function () {
                        location.href = "painel.php?exe=posts/posts&post=" + id + "&action=active";
                    }, 200);
                } else {
                    setTimeout(function () {
                        location.href = "painel.php?exe=posts/posts&post=" + id + "&action=inative";
                    }, 200);
                }
            }
